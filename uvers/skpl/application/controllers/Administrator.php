...
class User extends CI_Controller
{
	...
	public function index($id = 1)
	{
		$data = array(
			'title' => 'Data User',  
			'head' => '_partials/head',
			'konten' => 'v_admin-administrator',
			'pengaturan' => $this->ptn->ambil($id)->result(),
			'administrator' => $this->usr->ambildata()->result()
		);
	...
	public function profil($id = 1)
	{
		...
		$data = array(
			...
			'konten' => 'v_profil',
			'pengaturan' => $this->ptn->ambil($id)->result(),
			'administrator' => $this->usr->ambil($id_administrator)->result()
		);
	...
	{
		$where = $this->input->post('id_administrator');
		$data = array(
			'nama' => $this->input->post('nama'),
	...
	public function update_profil()
	{
		...
		$administrator = $this->usr->ambil($where)->result();
		$nama = $administrator[0]->nama;
		$email = $administrator[0]->email;
		$hp = $administrator[0]->hp;
		...
		// kembali ke halaman sebelumnya sesuai dengan masing-masing administrator dengan akses yang berbeda
		redirect($_SERVER['HTTP_REFERER']);
	}
	...
	public function update_password()
	{
		...
		if ($cek_id->num_rows() > 0) {
			$administrator = $cek_id->result();
			$cekpass = $administrator[0]->password;
	...
	public function hapus($id_administrator = null)
	{
		$hapus = $this->usr->hapus($id_administrator);
	...
	public function ceklogin()
	{
		...
		if ($ceknama->num_rows() > 0) {
			$administrator = $ceknama->result();
			$cekpass = $administrator[0]->password;

			// memverifikasi password dengan password di database
			if (password_verify($password, $cekpass)) {
				$id_administrator = $administrator[0]->id_administrator;
				$nama = $administrator[0]->nama;
				$email = $administrator[0]->email;
				$hp = $administrator[0]->hp;
				$level = $administrator[0]->level;

				$this->session->set_userdata('id_administrator', $id_administrator);
				$this->session->set_userdata('nama', $nama);
				$this->session->set_userdata('email', $email);
				$this->session->set_userdata('hp', $hp);
